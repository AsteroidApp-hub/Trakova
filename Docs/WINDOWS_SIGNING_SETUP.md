...
1. <https://signpath.org/apply.html> から OSS 申請する。
2. 申請時に提示する情報:
   - **リポジトリ**: `https://github.com/AsteroidApp-hub/Utawave` (公開・AGPL-3.0-or-later)
   - **ライセンス**: AGPLv3 (OSI 承認・商用デュアルライセンスでない) → 条件を満たす
   - プロジェクト概要 (歌い手向け録音 DAW・無料配布)
3. 承認されると SignPath.io 上に Foundation のプロジェクトが作られ、組織 (Organization) に招待される。
4. **チーム全員が SignPath と GitHub の両方で MFA (多要素認証) を有効化**しておくこと (Foundation の必須条件)。
