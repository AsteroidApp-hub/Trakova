...
// Trakova — 世代バックアップ管理 (BackupManager) のユニットテスト
//
...
public:
    ...
    void runTest() override
    {
        dir = juce::File::getSpecialLocation(juce::File::tempDirectory)
                  .getChildFile("TrakovaBackupMgrTests");
        projFile = juce::File::getSpecialLocation(juce::File::tempDirectory)
                       .getChildFile("TrakovaBackupMgrProj.trakova");
    ...
    // ── 2. 日時付きファイル名 = <prefix><YYYYMMDD_HHMMSS>.trakova ──
    void testDatedFileName()
    {
        beginTest("datedFile: structure prefix + 15-char stamp + .trakova (timezone-independent)");
        freshDir();
        ...
        expect(nm.endsWith(".trakova"), "ends with extension");

        auto stamp = nm.fromFirstOccurrenceOf("Song.autosave-", false, false)
                       .upToLastOccurrenceOf(".trakova", false, false);
    ...
    void testListMatchesAndFilters()
    {
        ...
        freshDir();
        auto dated  = makeFile("Song.autosave-20230101_000000.trakova", at(10));
        auto legacy = makeFile("Song.autosave.trakova",                 at(5));
        makeFile("Song.trakova",                          at(20));   // 本体ファイル相当 → 除外
        makeFile("SongX.autosave-20230101_000000.trakova", at(20));  // 別プロジェクト → 除外
        makeFile("Songbook.autosave-20230101_000000.trakova", at(20)); // 接頭辞境界 → 除外
    ...
    void testListOrderingNewestFirst()
    {
        ...
        freshDir();
        auto a = makeFile("Song.autosave-20230101_000001.trakova", at(10));
        auto b = makeFile("Song.autosave-20230101_000002.trakova", at(30));
        auto c = makeFile("Song.autosave-20230101_000003.trakova", at(20));
    ...
    void testNewest()
    {
        ...
        makeFile("Song.autosave-20230101_000001.trakova", at(10));
        auto top = makeFile("Song.autosave-20230101_000002.trakova", at(50));
        makeFile("Song.autosave-20230101_000003.trakova", at(30));
    ...
    void testPruneKeepsNewest()
    {
        ...
        for (int i = 0; i < 5; ++i)   // i が大きいほど新しい
            made.add(makeFile("Song.autosave-2023010" + juce::String(i + 1) + "_000000.trakova",
                              at(i * 100)));
    ...
    void testWildcardMetacharSafety()
    {
        ...
        expect(! makeFile("My*Mix.autosave-20230101_000000.trakova", at(10)).existsAsFile(),
               "Windows rejects '*' in filenames; wildcard scenario is not applicable");
       #else
        auto starFile = makeFile("My*Mix.autosave-20230101_000000.trakova", at(10));
        ...
        makeFile("MyXMix.autosave-20230101_000000.trakova", at(20));
    ...
    void testUntitledPrefix()
    {
        ...
        freshDir();
        auto dated  = makeFile("autosave-20230101_000000.trakova", at(10));
        auto legacy = makeFile("autosave.trakova",                 at(5));
        makeFile("Song.autosave-20230101_000000.trakova", at(20));   // 名前付き → 未保存では除外
    ...
    void testShouldOfferRecovery()
    {
        ...
        // (b) バックアップが本体より古い
        makeFile("Song.autosave-20230101_000000.trakova", at(50));
        expect(! BackupManager::shouldOfferRecovery(dir, "Song", projFile),
               "older backup: do not offer");

        // (c) 同時刻 (境界 <=)
        freshDir();
        auto eq = makeFile("Song.autosave-20230101_000001.trakova", at(100));
        ...
        // (d) バックアップが本体より新しい
        freshDir();
        makeFile("Song.autosave-20230101_000002.trakova", at(150));
        expect(BackupManager::shouldOfferRecovery(dir, "Song", projFile),
