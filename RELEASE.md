...
ビルドは CI ではなく **各自のマシンで手動**で行い、生成した zip を**公式サイトリポジトリ
(Utawave-Site) の `dl/` にコミット**して Cloudflare Pages から配信します
（配置ルールの詳細はサイト側 README の「配布ファイルの置き方」を参照。
1 ファイル 25MiB 以内・Git LFS 不使用・旧バージョンは並置）。
（ASIO 対応の Windows 版もこの手動ビルドで作れます）

zip のファイル名にはバージョンを含めます（例: `Utawave-0.1.0-macOS.zip` /
`Utawave-0.1.0-win64.zip`）。以下のコマンド例はリリースするバージョンに読み替えてください。
...
ditto -c -k --sequesterRsrc --keepParent Utawave-macOS Utawave-0.1.0-macOS.zip
```
...
Compress-Archive -Path Utawave-Windows -DestinationPath Utawave-0.1.0-win64.zip -Force
```
...
## 5. GitHub Release を作成（タグ + 配布物のバックアップ）
...
## 6. 公式サイトに公開（一般利用者向けの配布先）

1. **公式サイトリポジトリ (Utawave-Site) で**:
   - 手順 2・3 で作った zip を `dl/` に追加（**1 ファイル 25MiB 以内**であることを確認）
